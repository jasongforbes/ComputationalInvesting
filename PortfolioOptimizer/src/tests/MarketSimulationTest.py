...
class MarketSimulationTest(unittest.TestCase):
    ...
    def setUp(self):
        '''                 name          csv_files                                    output                    start-date         end-date                 symbols               '''
        self.test_data = [('orders', '../../Resources/orders.csv',       '../../Resources/output.csv',       dt.date(2011,1,10), dt.date(2011,12,20), ['AAPL','IBM','GOOG','XOM']), \
                          ('orders2','../../Resources/orders2.csv',       '../../Resources/output2.csv',     dt.date(2011,1,14), dt.date(2011,12,14), ['AAPL','IBM','GOOG','XOM']) \
                          ]
    ...
    def testLoadingCsv(self):
        for name, csvfile,outputcsv,startdate,enddate,symb in self.test_data:
    ...
    def testPortfolioAnalyzer(self):
         for name, csvfile, outputcsv, startdate, enddate, symb in self.test_data:
            orders = mo.MarketOrders(csvfile)
            portfolio = ps.PortfolioSimulation(orders,1000000)
            az.PortfolioAnalyzer().analyze(portfolio,'$SPX', name=name)
